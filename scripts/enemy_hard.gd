...
var health = 500 + Global.hard_enemy_health
var target: Node2D
var speed = 150
...
func melee_hit():
	...
	if health > 0:
		$PlayerHit/AttackHitbox.disabled = true
		health -= (Global.meelee_damage * 2.5) * Global.damage_buff
		is_hit = true
...
func pistol_hit():
	hit.play()
	health -= (Global.pistol_damage * 8) * Global.damage_buff
...
func smg_hit():
	hit.play()
	health -= Global.smg_damage * Global.damage_buff
	is_hit = true  # Set hit flag to true
	$AnimationPlayer.play("hit")
	enemy_health()
	$AttackCooldown.start() # Start cooldown to reset hit
...
func explosive_hit():
	var dogde = Global.rng.randi_range(1,10)
	if not dogde <= 2:
		hit.play()
		health -= (Global.grenade_damage * 2) * Global.damage_buff
		is_hit = true  # Set hit flag to true
		$AnimationPlayer.play("hit")
		enemy_health()
		$AttackCooldown.start()  # Start cooldown to reset hit
